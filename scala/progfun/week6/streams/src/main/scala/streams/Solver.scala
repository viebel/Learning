...
trait Solver extends GameDef {
  ...
  def newNeighborsWithHistory(b: Block, history: List[Move], explored: Set[Block]): Stream[(Block, List[Move])] = 
  ...
  def from(initial: Stream[(Block, List[Move])],
           explored: Set[Block]): Stream[(Block, List[Move])] = 
   {
     ...
     lazy val initial_blocks = initial.map(_._1)
     def neighbors = for {
       (block, history) <- initial
       neighbor <- newNeighborsWithHistory(block, history, explored ++ initial_blocks)
     } yield neighbor
     initial #::: from(neighbors, explored ++ initial_blocks)
     }
   }
